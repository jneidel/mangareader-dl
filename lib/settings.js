...
function outputHistory( settings ) {
  ...
  if ( mangas.length > 0 ) {
    ...
    mangas.forEach( manga => console.log( `  ${chalk.green( mangaObj[manga].subscribe ? "✓" : " " )} ${strpad.right( manga, longestName )} ${strpad.left( chalk.green( mangaObj[manga].chapter ), 13 )} [${strpad.right( mangaObj[manga].provider, "mangareader".length )} ${mangaObj[manga].path}]` ) );
  } else {
