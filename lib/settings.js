...
function outputHistory( settings, isLatest = false ) {
  ...
  function printManga( manga, isLastChapter = false ) {
    log.print( `  ${mangaObj[manga].subscribe ? isLastChapter ? chalk.green( "✓" ) : chalk.red( "✓" ) : " "} ${strpad.right( manga, longestName )} ${strpad.left( isLastChapter ? chalk.green( mangaObj[manga].chapter ) : chalk.red( mangaObj[manga].chapter ), 13 )} [${strpad.right( mangaObj[manga].provider, "mangareader".length )} ${mangaObj[manga].path}]` );
  }
    ...
    mangas.forEach( async manga => {
      if ( mangaObj[manga].subscribe !== undefined && mangaObj[manga].chapter !== undefined && mangaObj[manga].path !== undefined && mangaObj[manga].provider !== undefined ) {
        if ( isLatest ) {
          ...
          if ( lastChapter === mangaObj[manga].chapter )
            printManga( manga, true );
          else
            printManga( manga );
        } else
          printManga( manga );
