...
function outputHistory( settings, isLatest = false ) {
  ...
  function printManga( manga, isLatest = true ) {
    log.print( `  ${mangaObj[manga].subscribe ? isLatest ? chalk.green( "✓" ) : chalk.red( "✓" ) : " "} ${strpad.right( manga, longestName )} ${strpad.left( isLatest ? chalk.green( mangaObj[manga].chapter ) : chalk.red( mangaObj[manga].chapter ), 13 )} [${strpad.right( mangaObj[manga].provider, "mangareader".length )} ${mangaObj[manga].path}]` );
  }
