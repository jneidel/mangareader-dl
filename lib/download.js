...
const progressBar = {
  ...
  lastTimePad: ( last, all ) => `last: ${last / 60 >= 1 ? `${( last / 60 ).toFixed( 0 )}:` : all / 60 <= 1 ? "" : all / 60 <= 10 ? "  " : all / 60 <= 100 ? "   " : "    "}${last / 60 >= 1 ? strpad.left( ( last / 60 % 1 * 100 ).toFixed( 0 ), 2, "0" ) : all < 60 && all > 10 && last < 10 ? strpad.left( last.toFixed( 0 ), 2 ) : last.toFixed( 0 )}${last / 60 >= 1 ? "m" : "s"}`,
 ...
 */
async function downloadChapters( manga, isExt, settings ) {
  ...
  if ( nextChapter ) {
    return downloadChapters( nextChapter, isExt, settings );
  }
...
async function downloadManga( url, options, settings ) {
  ...
  if ( options.isExt ) { // --extended progress bar
    globalState.done = false;
    var bar = progressBar.chapter.setup( manga );
    progressBar.chapter.runUpdate( bar );
  }

  await downloadChapters( manga, options.isExt, settings )
    .catch( err => {
      console.log( err );
      downloadManga( err.message, options, settings );
