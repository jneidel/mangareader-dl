 ...
 */
async function downloadManga( url, options ) {
  const manga = await prepareDownload( url, options );
  ...
  await downloadChapters( manga, options.isExt )
    .catch( err => {
      console.log( err );
      downloadManga( err.message, options );
