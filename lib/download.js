...
const progressBar = {
  ...
  page       : {
    ...
    setup( manga, isExt ) {
      ...
      return progress( { width: barLen, total: manga.pagesTotal, incomplete: "░", complete: "█" } );
    },
    /**
    ...
    update( instance, manga, spinnerFrame, pagesCurrent, isExt = false ) {
      ...
      logUpdate( `${pb.spinner( spinnerFrame )} ${manga.name} ${chalk.green( manga.chapter )} [${instance( pagesCurrent )}] ${pb.percent0( pagesCurrent, manga.pagesTotal )} | ${pb.pageProg( pagesCurrent, manga.pagesTotal )} | ${pb.chProg( manga.chapter, manga.chapterTotal )}` );
     ...
     */
    finish( instance, manga, downloadTimer, isExt ) {
...
async function prepareDownload( url, outputPath, provider, isForce ) {
  ...
  if ( manga.chapter > manga.chapterTotal ) { // Historic chapters has been ++, check if valid
    return i.prependArrowPrintStdout( `${name} has no chapters available for download. (Latest: ${manga.chapter}). \n  If you want to overwrite the history and download with specified chapter/path use the --force flag.` );
  }
