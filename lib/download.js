...
async function downloadChapters( manga, isMin ) {
  ...
  i.writeHistory( { name: manga.name, chapter: manga.chapter, path: manga.outputPath } );
 ...
 * @param {boolean} isMin - Whenever the --min flag has been passed
 */
async function downloadManga( url, outputPath, isForce, isMin ) {
  if ( isForce ) {
    var manga = await i.createManga( url, outputPath );
    ...
    manga.siteUrl = i.createSiteUrl( manga.name, manga.chapter ); // SiteUrl with correct chapter
  ...
  await downloadChapters( manga, isMin )
    .catch( err => {
      console.log( err );
      downloadManga( err.message, outputPath, isForce, isMin );
