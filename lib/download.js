...
const progressBar = {
  ...
  page   : {
    ...
    update( instance, manga, spinnerFrame, pagesCurrent, barMode ) {
      ...
      log.update( `${pb.spinner( spinnerFrame )} ${manga.name} ${chalk.green( manga.chapter )} [${instance( pagesCurrent )}] ${pb.percent0( pagesCurrent, manga.pagesTotal )} | ${pb.pageProg( pagesCurrent, manga.pagesTotal )} | ${pb.chProg( manga.chapter, manga.chapterTotal )}${globalState.timer.last ? ` | ${pb.lastTime( globalState.timer.last )}` : ""}` );
    },
  ...
  chapter: {
    ...
    setup( manga ) {
      ...
      globalState.chapterStart = manga.chapter;
      globalState.chapter = manga.chapter;
...
async function prepareDownload( url, { outputPath, provider, isForce, subscribe }, settings ) {
  log.update( `${chalk.green( "❯" )} Preparing the download...` );
