...
async function downloadChapters( manga ) {
  ...
  console.log( "write history", { name: manga.name, chapter: manga.chapter, path: manga.outputPath } );
  i.writeConfig( { name: manga.name, chapter: manga.chapter, path: manga.outputPath } );
...
async function downloadManga( url, outputPath ) {
  ...
  const { chapter, mangaPath } = i.readHistory( name );

  outputPath = chapter ? mangaPath : outputPath; // Reassign path from history

  const manga = await i.createManga( url, outputPath );
  manga.chapter = chapter ? chapter : manga.chapter; // Reassign chapter from history
