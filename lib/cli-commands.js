...
async function update( args, settings ) {
  ...
  for ( const manga of mangaToDownload ) {
    const options = {
  ...
  const mangaStr = mangaToDownload.reduce( ( arr, manga ) => {
    const name = manga.name;
    const start = manga.chapter;
    const { chapter: end } = s.readHistoryForName( settings, name );

    return `${arr}\n    - ${name} (${start + 3 > end ? range( start, end + 1 ) : `${start}-${end}`})`;
  }, "" );

  if ( mangaToDownload.length )
    setTimeout( () => {
