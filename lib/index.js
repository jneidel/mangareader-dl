...
function parseFromUrl( url, passedProvider = null ) {
  let [ , provider, name, chapter = 1, page = 1 ] =
    String( url ).match( /(?:https?:\/\/)?(?:www.)?((?:mangareader.net)|(?:readmng.com)|(?:goodmanga.net)|(?:mangapanda.com)|(?:mangainn.net))?(?:\/)?([^/]+)\/?(?:chapter\/)?(\d+)?\/?(\d+)?/i );
  /** Matches:
...
const downloadImg = ( manga ) => {
  ...
  return providerLib.getImgBuffer( manga )
    .then( buffer => { timeout.reset(); return buffer; } )
    .catch( err => {
      timeout.add( `Connection Error: Downloading image from the url '${manga.imgSrc}'` );
      console.log( err );
