...
function parseFromUrl( url, passedProvider = null ) {
  let [ , provider, name, chapter = 1, page = 1 ] =
    String( url ).match( /(?:https?:\/\/)?(?:www.)?((?:mangareader.net)|(?:readmng.com)|(?:goodmanga.net)|(?:mangapanda.com)|(?:mangainn.net)|(?:mangalife.us\/read-online)|(?:mangalife.us\/manga))?(?:\/)?([^/]+)\/?(?:chapter\/)?(\d+)?\/?(\d+)?/i );
  /** Matches:
   ...
   * http://www.mangalife.us/manga/kemono-jihen
   */
