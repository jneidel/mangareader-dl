 ...
 */
const getImgSrcIfValid = ( url, provider ) => axios.get( url )
  .then( html => {
 ...
 */
const downloadImg = imgSrc => axios.get( imgSrc, { responseType: "arraybuffer", timeout: 1000 } )
  .then( res => res.data )
...
const getLastPage = url => axios.get( url )
  .then( html => {
    const $ = cheerio.load( html.data );
