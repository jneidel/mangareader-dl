...
const progressBar = {
  ...
  page   : {
    ...
    setup( manga, barMode, gs ) {
      ...
      if ( barMode === "extended" ) return progressBar.page.setupExt( manga, gs );
    ...
    update( instance, manga, spinnerFrame, pagesCurrent, barMode, gs ) {
      ...
      if ( barMode === "extended" ) return progressBar.page.updateExt( spinnerFrame, pagesCurrent, gs );

      const pb = progressBar;

      log.update( `${pb.spinner( spinnerFrame )} ${manga.name} ${chalk.green( manga.chapter )} [${instance( pagesCurrent )}] ${pb.percent0( pagesCurrent, manga.pagesTotal )} | ${pb.pageProg( pagesCurrent, manga.pagesTotal )} | ${pb.chProg( manga.chapter, manga.chapterTotal )}${gs.timer.last ? ` | ${pb.lastTime( gs.timer.last )}` : ""}` );
    },
    updateMic( manga, pagesCurrent, spinnerFrame ) {
      const pb = progressBar;

      log.update( `${pb.spinner( spinnerFrame )} ${manga.name} ${chalk.green( `${manga.chapter}/${manga.chapterTotal}` )} ${( pagesCurrent / manga.pagesTotal * 100 ).toFixed( 0 )}%` );
    },
    /**
     * Update page progress bar by updating the changed values in globalState
     */
    updateExt( spinnerFrame, pagesCurrent, gs ) {
    ...
    finish( instance, manga, lastDownloadTimer, barMode, gs ) {
      ...
      if ( barMode === "extended" ) return progressBar.page.finishExt( gs );
